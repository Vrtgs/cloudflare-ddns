...
async fn ipc_channel(child: &mut Child) -> Result<(RecvHalf, SendHalf)> {
    let path = {
        ...
        let Some(b'\0') = temp_buf.pop()
            else { anyhow::bail!("could not read path size from child") };

        let sz = atoi::atoi::<u64>(&temp_buf).with_context(|| {
            "invalid length provided by child"
        })?;

        let sz = usize::try_from(sz)?;
        temp_buf.clear(); temp_buf.reserve(sz);
...
impl WasmDriver {
    async fn read_response<R: AsyncBufRead + Unpin>(stream: &mut R) -> Result<Option<Response>> {
        ...
        stream.take(resp_len as u64).read_to_end(&mut resp_buffer).await?;
        anyhow::ensure!(resp_buffer.len() == resp_len, "child provided invalid response length");

        let (resp, _) = bincode::decode_from_slice::<Response, _>(&resp_buffer, BIN_CODE_CONFIG)?;
        Ok(Some(resp))
    }

    async fn write_command<W: AsyncWrite + Unpin>(stream: &mut W, cmd: WasmCommand<'_>) -> Result<()> {
    ...
    async fn _open(path: &Path) -> Result<Self> {
        ...
        let (recv, mut send) = timeout(
            ...
            ipc_channel(&mut child)
        ).await.with_context(|| "could not connect to child.. timed out")??;
        ...
        let write_task = tokio::spawn(async move {
            ...
            while let Some((module, data, recv)) = receiver.recv().await {
                ...
                Self::write_command(&mut send, WasmCommand::Request(Request {
                    id: entry_guard.id,
                    module: &module,
                    data: &data,
                })).await?;
        ...
        Ok(Self(Some(WasmDriverInner { write_task, read_task, sender })))
    }
